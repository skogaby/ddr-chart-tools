...
7. **Default output is under the input tree; `--output-dir` overrides.** Single-file conversions write to `./output` by default; batch writes to `<input-folder>/output`. When `--output-dir` is provided, it is used verbatim in both modes. Output filenames are derived deterministically from input filenames (the `_all` suffix used by Ultramix is stripped), except that an explicit `--song-code` (single-file `DDR` output only) names the outputs instead — see rule 11.
8. **File pairing.** In batch mode, a chart file and its corresponding audio file are paired by shared basename (e.g. `foo.ssq` ↔ `foo.xwb`, `bar.ssc` ↔ `bar.ogg`). For `DDR_LEGACY` inputs, the chart's `_all` suffix is stripped during pairing so Ultramix charts match their audio (`abs2_all.ssq` ↔ `abs2.wavm`). When several chart files share a basename (`song.ssc` + `song.sm`), the format's preferred extension wins (`.ssc` over `.sm`) regardless of directory order. Unpaired files are skipped with a warning.
...
11. **The song code is the output basename.** For `DDR` output, the XACT wave bank name and cue names (`{code}`, `{code}_s`) equal the basename the `.ssq`/`.xwb`/`.xsb` are written under — the game resolves the cue by a byte-exact compare against the ID it loads the files by, so any other relationship produces silent audio. `--song-code` sets both; otherwise the input stem is used as-is when it is a valid code (1–16 ASCII alphanumerics), and a best-effort short code plus a warning otherwise.
12. **Every `#BPMS` entry is encoded.** SSQ stores tempo as the slope between consecutive tempo pairs, so the SM5→DDR tempo chunk always ends with a trailing pair placed *after* the last segment boundary (at END's tick). Extrapolating the trailing pair from the preceding pair-pair is only acceptable for legacy sources whose raw pairs are passed through.
